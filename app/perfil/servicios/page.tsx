...
interface ServiceItem {
  ...
  description: string;
  price: number;
  currency?: string; // Campo opcional por si deseas estructurarlo luego en Supabase
}
...
export default function ServiciosPage() {
  ...
  useEffect(() => {
    async function inicializarServicios() {
      ...
      if (artist) {
        ...
        if (items) setServicesItems(items);
      }
      setLoading(false);
    }
  ...
  // Función para formatear a dos decimales al salir del input
  ...
  const agregarServicio = async (e: React.FormEvent) => {
    ...
    const nuevoServicio = {
      ...
      // Nota: Si agregas la columna 'currency' en tu tabla de Supabase, puedes descomentar la siguiente línea:
      // currency, 
    };
    ...
    <div className="p-8 max-w-2xl mx-auto text-white space-y-8">
      ...
      <section className="space-y-4 bg-zinc-950 p-6 rounded-xl border border-zinc-800">
        <header className="border-b border-zinc-800 pb-3">
          ...
          <p className="text-zinc-400 text-xs mt-1">Añade prestaciones musicales o talleres a tu portafolio.</p>
        </header>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-2">
              ...
              <input 
                ...
                onBlur={formatearDecimales} // Formatea automáticamente al hacer clic afuera
                className="w-full bg-zinc-900 border border-zinc-700 rounded p-2 text-sm text-white focus:outline-none focus:border-amber-500"
                placeholder="150.00"
                required
              />
              ...
              <div key={service.id} className="bg-zinc-900 p-5 rounded-lg border border-zinc-800 flex flex-col justify-between space-y-3">
                <div>
                  <div className="flex justify-between items-start">
                    ...
                    {/* Renderiza dinámicamente el símbolo basado en lo guardado (por defecto maneja el formato visual) */}
                    <span className="text-base font-bold text-emerald-400">
                      {currency === "PEN" ? "S/." : "$"}{Number(service.price).toFixed(2)}
                    </span>
                  </div>
